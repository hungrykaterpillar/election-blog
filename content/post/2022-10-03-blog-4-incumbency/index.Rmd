...
This first map visualizes Democratic voteshare at the district level in 2018, based on the Democrat vote share percentage. The map holds many districts that are blue and those that lean towards Democrats. There seems to be a larger voteshare for Democrats (which is in their favor) at the district level. The districts which voted red during the 2014 midterms (California's 4th district, Washington's 4th district, and Texas's 11th district) all had visibly increased Democrat voteshare. An exception to this trend of previously Republican leaning districts shifting towards Democrats is California's 8th district, which had a record-high GOP voteshare. Even though Democrats improved their performance compared to the 2014 midterms and the 2016 presidential elections and had a couple strong Democrat districts, such as Mississippi's 2nd district and Alabama's 7th district, it is clear from the map above that the nationwide popularity for Republicans was still persistent in 2018. 
...
Looking closer into the model using Democratic Majority vote Percentage Lag, GDP Growth Percentage, and U.S. CPI used to predict Democratic seat share, the model's predictive interval ranges from about 150 to 245 seats, which includes the fitted estimate of Democrat seat share in the 2018 House of Representatives elections (197 seats), which was a shocking 38 seats higher than the actual seat share of the Democrat Party in that election. 
```{r, echo = FALSE}
##lm_incumb_D1 <- lm(DemVotes ~ incumbent_party+open_seat, data = HVS_data)
...
type="l", main="GDP Model", xlab = "Election Year", ylab = "House Incumbent Majority Vote %")
